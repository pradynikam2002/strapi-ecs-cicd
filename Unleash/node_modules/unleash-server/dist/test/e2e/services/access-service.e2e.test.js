...
import { ALL_PROJECTS, CUSTOM_ROOT_ROLE_TYPE, DEFAULT_ENV, } from '../../../lib/util/constants.js';
import { createAccessService, createFeatureToggleService, createProjectService, } from '../../../lib/features/index.js';
...
const createUser = async (role) => {
    ...
    if (role)
        await accessService.addUserToRole(user.id, role, role === readRole.id ? ALL_PROJECTS : DEFAULT_PROJECT);
    return user;
};
const groupIndex = 0;
...
test('should remove CREATE_FEATURE on default environment', async () => {
    ...
    await accessService.addPermissionToRole(editRole.id, permissions.CREATE_FEATURE, DEFAULT_ENV);
...
test('should switch project roles on when multiple roles are present for same user', async () => {
    ...
    await accessService.updateUserProjectRole(userOne.id, targetRole.id, DEFAULT_PROJECT);
});
...
test('should support permission with "ALL" environment requirement', async () => {
    ...
    await accessStore.addPermissionsToRole(customRole.id, [{ name: CREATE_FEATURE_STRATEGY }], DEFAULT_ENV);
    await accessStore.addUserToRole(user.id, customRole.id, ALL_PROJECTS);
    const hasAccess = await accessService.hasPermission(user, CREATE_FEATURE_STRATEGY, 'default', DEFAULT_ENV);
    expect(hasAccess).toBe(true);
    const hasNotAccess = await accessService.hasPermission(user, CREATE_FEATURE_STRATEGY, 'default', 'production');
    expect(hasNotAccess).toBe(false);
});
test('Should have access to create a strategy in an environment', async () => {
    const { CREATE_FEATURE_STRATEGY } = permissions;
    const user = editorUser;
    expect(await accessService.hasPermission(user, CREATE_FEATURE_STRATEGY, 'default', DEFAULT_ENV)).toBe(true);
});
...
test('Should have access to edit a strategy in an environment', async () => {
    ...
    const user = editorUser;
    expect(await accessService.hasPermission(user, UPDATE_FEATURE_STRATEGY, 'default', DEFAULT_ENV)).toBe(true);
});
test('Should have access to delete a strategy in an environment', async () => {
    const { DELETE_FEATURE_STRATEGY } = permissions;
    const user = editorUser;
    expect(await accessService.hasPermission(user, DELETE_FEATURE_STRATEGY, 'default', DEFAULT_ENV)).toBe(true);
});
...
test('Should be denied access to delete a role that is in use', async () => {
    ...
    [projectMember.id], SYSTEM_USER_AUDIT);
    try {
...
test('Should be denied move feature flag to project where the user does not have access', async () => {
    ...
    await featureToggleService.createFeatureToggle(projectOrigin.id, featureFlag, extractAuditInfoFromUser(user));
    try {
        await projectService.changeProject(projectDest.id, featureFlag.name, user, projectOrigin.id, TEST_AUDIT_USER);
    }
    catch (e) {
...
test('Should be allowed move feature flag to project when the user has access', async () => {
    ...
    await projectService.createProject(projectOrigin, user, extractAuditInfoFromUser(user));
    await projectService.createProject(projectDest, user, extractAuditInfoFromUser(user));
    const featureFlag = { name: 'moveableFlag2' };
    await featureToggleService.createFeatureToggle(projectOrigin.id, featureFlag, extractAuditInfoFromUser(user));
    await projectService.changeProject(projectDest.id, featureFlag.name, user, projectOrigin.id, extractAuditInfoFromUser(user));
});
...
test('Should allow user to take multiple group roles and have expected permissions on each project', async () => {
    ...
    const viewerUser = await createUser(readRole.id);
    await projectService.createProject(projectForCreate, editorUser, TEST_AUDIT_USER);
    await projectService.createProject(projectForDelete, editorUser, TEST_AUDIT_USER);
...
test('calling add access with invalid project role ids should not assign those roles', async () => {
    ...
    await expect(() => accessService.addAccessToProject([adminRootRole.id, 9999], [], [emptyUser.id], projectName, 'some-admin-user')).rejects.toThrow(BadDataError);
    const newAssignedPermissions = await accessService.getPermissionsForUser(emptyUser);
