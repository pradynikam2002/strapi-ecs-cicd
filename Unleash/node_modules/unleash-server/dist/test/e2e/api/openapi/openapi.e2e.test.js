import { setupApp, setupAppWithBaseUrl, } from '../../helpers/test-helper.js';
...
describe('subpath handling', () => {
    ...
    test('the OpenAPI spec has the base path appended to its server', async () => {
        const { body: { servers }, } = await appWithSubPath.request
    ...
    test('When the server has a base path, that base path is stripped from the endpoints', async () => {
        const { body: { paths }, } = await appWithSubPath.request
            .get(`${subPath}/docs/openapi.json`)
            .expect('Content-Type', /json/)
            .expect(200);
...
test('all tags are listed in the root "tags" list', async () => {
    ...
    if (Object.keys(invalidTags).length) {
        ...
        const msgs = Object.entries(invalidTags).flatMap(([path, data]) => Object.entries(data).map(([operation, opData]) => `${operation.toUpperCase()} ${path} (operation id: ${opData.operationId}) has the following invalid tags: ${opData.invalidTags
            .map((tag) => `"${tag}"`)
            .join(', ')}`));
        // format message
...
test('all API operations have non-empty summaries and descriptions', async () => {
    ...
    const anomalies = Object.entries(spec.paths).flatMap(([path, data]) => {
        return Object.entries(data)
            ...
            if (operationDescription.summary &&
        ...
        ([verb, operationId]) => `${verb.toUpperCase()} ${path} (operation ID: ${operationId})`);
    });
    // any items left in the anomalies list is missing either a summary, or a
    // description, or both.
    expect(anomalies).toStrictEqual([]);
});
//# sourceMappingURL=openapi.e2e.test.js.map