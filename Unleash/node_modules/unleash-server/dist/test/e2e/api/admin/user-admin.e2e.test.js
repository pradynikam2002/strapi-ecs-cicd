...
describe('Users created without an event are amended', () => {
    ...
    test('should amend users created without an event', async () => {
        ...
        const userCreatedEvent = eventsPostMigrations.find((e) => e.type === USER_CREATED && e.data.id === insertedUser.id);
...
describe('User Admin API without email', () => {
    ...
    test('When invalid role is provided the user is not created', async () => {
        ...
        return app.request
            ...
            .expect(200)
            .expect((res) => {
            expect(res.body.users).not.toContainEqual(expect.objectContaining({
                email: 'should-not-exist@getunleash.ai',
            }));
        });
    });
    ...
    test('Anonymises name, username and email fields if anonymiseEventLog flag is set', async () => {
        const anonymisedApp = await setupAppWithCustomConfig(stores, { experimental: { flags: { anonymiseEventLog: true } } }, db.rawDatabase);
    ...
    test('should only delete scim users', async () => {
        ...
        expect(users.every((u) => u.email !== 'made-by-scim@example.com')).toBe(true);
    });
});
