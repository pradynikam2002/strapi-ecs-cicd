...
test('returns three archived flags', async () => {
    expect.assertions(1);
    return app.request
        .get(`/api/admin/search/features?project=IS%3A${DEFAULT_PROJECT}&archived=IS%3Atrue`)
...
test('Should be able to bulk archive features', async () => {
    ...
    const { body } = await app.request
        .get(`/api/admin/search/features?project=IS%3A${DEFAULT_PROJECT}&archived=IS%3Atrue`)
        .expect(200);
    const archivedFeatures = body.features.filter((feature) => feature.name === featureName1 || feature.name === featureName2);
...
//# sourceMappingURL=feature-archive.e2e.test.js.map