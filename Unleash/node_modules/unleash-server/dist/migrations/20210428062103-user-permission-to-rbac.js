...
exports.up = function (db, cb) {
    db.runSql('SELECT id, permissions from users WHERE id NOT IN (select user_id from role_user);', (err, results) => {
        if (results.rowCount > 0) {
            const users = results.rows;
            const insertRootRole = users.map((u) => {
                const roleName = resolveRoleName(u.permissions);
                return db.runSql.bind(db, `INSERT INTO role_user (role_id, user_id)
                     SELECT id, '${u.id}'
                     FROM roles
...
//# sourceMappingURL=20210428062103-user-permission-to-rbac.js.map