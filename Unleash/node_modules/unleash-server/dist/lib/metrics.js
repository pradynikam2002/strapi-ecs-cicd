...
import { DB_POOL_UPDATE, FEATURE_ARCHIVED, FEATURE_CREATED, FEATURE_REVIVED, FEATURE_STRATEGY_ADD, FEATURE_STRATEGY_REMOVE, FEATURE_STRATEGY_UPDATE, FEATURE_ENVIRONMENT_ENABLED, FEATURE_ENVIRONMENT_DISABLED, FEATURE_VARIANTS_UPDATED, FEATURE_METADATA_UPDATED, FEATURE_UPDATED, CLIENT_METRICS, CLIENT_REGISTER, PROJECT_ENVIRONMENT_REMOVED, PROJECT_CREATED, PROJECT_ARCHIVED, PROJECT_REVIVED, PROJECT_DELETED, RELEASE_PLAN_ADDED, RELEASE_PLAN_REMOVED, RELEASE_PLAN_MILESTONE_STARTED, } from './events/index.js';
import { hoursToMilliseconds, minutesToMilliseconds } from 'date-fns';
import { createCounter, createGauge, createSummary, createHistogram, } from './util/metrics/index.js';
import { DbMetricsMonitor } from './metrics-gauge.js';
import { HEAP_MEMORY_TOTAL, REQUEST_COUNT, } from './features/metrics/impact/define-impact-metrics.js';
export function registerPrometheusPostgresMetrics(db, eventBus, postgresVersion) {
    if (db?.client) {
...
export function registerPrometheusMetrics(config, stores, version, eventBus, instanceStatsService) {
    ...
    const cachedEnvironments = memoizee(async () => environmentStore.getAll(), {
        promise: true,
    ...
    dbMetrics.registerGaugeDbMetric({
        ...
        map: (result) => result.map(({ domain, count }) => ({
            value: count,
            labels: {
                domain,
            },
    ...
    eventBus.on(events.REQUEST_TIME, ({ path, method, time, statusCode, appName }) => {
        requestDuration
            ...
            .observe(time);
        config.flagResolver.impactMetrics?.incrementCounter(REQUEST_COUNT, 1, { flagNames: ['consumptionModel'], context: {} });
    });
    ...
    eventStore.on(RELEASE_PLAN_ADDED, async ({ featureName, project, environment }) => {
        const environmentType = await resolveEnvironmentType(environment, cachedEnvironments);
        featureFlagUpdateTotal.increment({
            toggle: featureName,
            project,
    ...
    eventStore.on(RELEASE_PLAN_REMOVED, async ({ featureName, project, environment }) => {
        const environmentType = await resolveEnvironmentType(environment, cachedEnvironments);
        featureFlagUpdateTotal.increment({
            toggle: featureName,
            project,
    ...
    eventStore.on(RELEASE_PLAN_MILESTONE_STARTED, async ({ featureName, project, environment }) => {
        const environmentType = await resolveEnvironmentType(environment, cachedEnvironments);
        featureFlagUpdateTotal.increment({
            toggle: featureName,
            project,
    ...
    eventBus.on(CLIENT_METRICS, (metrics) => {
        try {
    ...
    eventStore.on(CLIENT_REGISTER, (heartbeatEvent) => {
    ...
    return {
        ...
        collectStaticCounters: async () => {
            try {
                config.flagResolver.impactMetrics?.updateGauge(HEAP_MEMORY_TOTAL, process.memoryUsage().heapUsed, { flagNames: ['consumptionModel'], context: {} });
...
export default class MetricsMonitor {
    ...
    async startMonitoring(config, stores, version, eventBus, instanceStatsService, schedulerService, db) {
        ...
        await schedulerService.schedule(async () => Promise.all([collectStaticCounters(), collectAggDbMetrics()]), hoursToMilliseconds(1), 'collectStaticCounters');
        await schedulerService.schedule(async () => this.registerPoolMetrics.bind(this, db.client.pool, eventBus), minutesToMilliseconds(1), 'registerPoolMetrics');
        return Promise.resolve();
    }
...
export function createMetricsMonitor() {
