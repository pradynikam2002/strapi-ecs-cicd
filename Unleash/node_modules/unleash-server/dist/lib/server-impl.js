...
import { createKnexTransactionStarter, withFakeTransactional, withRollbackTransaction, withTransactional, } from './db/transaction.js';
...
import { UPDATE_DELTA, } from './features/client-feature-toggles/delta/client-feature-toggle-delta.js';
...
import { addAjvSchema, validateSchema, } from './openapi/validate.js';
...
export async function initialServiceSetup({ authentication }, { userService, apiTokenService, }) {
...
export async function createApp(config, startApp, fm = {
    createDb,
    createStores,
    createServices,
    ...
    await metricsMonitor.startMonitoring(config, stores, serverVersion, config.eventBus, services.instanceStatsService, services.schedulerService, db);
    const unleash = {
        stores,
    ...
    return new Promise((resolve, reject) => {
        if (startApp) {
            const server = stoppable(app.listen(config.listen, () => logger.info('Unleash has started.', server.address())), config.server.gracefulShutdownTimeout);
