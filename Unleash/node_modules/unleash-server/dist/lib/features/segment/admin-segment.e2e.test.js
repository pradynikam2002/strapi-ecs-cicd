...
import { addStrategyToFeatureEnv, createFeatureFlag, } from '../../../test/e2e/helpers/app.utils.js';
...
const fetchSegments = () => app.request
    .get(SEGMENTS_BASE_PATH)
    .expect(200)
    .then((res) => res.body.segments);
const fetchSegmentsByStrategy = (strategyId) => app.request
    .get(`${SEGMENTS_BASE_PATH}/strategies/${strategyId}`)
    .expect(200)
    .then((res) => res.body.segments);
const fetchFeatures = () => app.request
...
const fetchFeatureStrategies = (featureName) => app.request
    ...
    .expect(200)
    .then((res) => res.body);
const fetchSegmentStrategies = (segmentId) => app.request
    .get(`${SEGMENTS_BASE_PATH}/${segmentId}/strategies`)
    .expect(200)
    .then((res) => res.body);
const updateSegment = (id, postData, expectStatusCode = 204) => app.request
    .put(`${SEGMENTS_BASE_PATH}/${id}`)
    .send(postData)
    .expect(expectStatusCode);
const addSegmentsToStrategy = (segmentIds, strategyId, expectStatusCode = 201) => app.request
...
const validateSegment = (postData, expectStatusCode = 204) => app.request
...
test('should not delete segments used by strategies', async () => {
    ...
    await addStrategyToFeatureEnv(app, { ...flag.strategies[0] }, DEFAULT_ENV, flag.name);
...
test('should list strategies by segment', async () => {
    ...
    await addStrategyToFeatureEnv(app, { ...flag1.strategies[0] }, DEFAULT_ENV, flag1.name);
    await addStrategyToFeatureEnv(app, { ...flag1.strategies[0] }, DEFAULT_ENV, flag2.name);
    await addStrategyToFeatureEnv(app, { ...flag3.strategies[0] }, DEFAULT_ENV, flag3.name);
    ...
    await addSegmentsToStrategy([segment1.id, segment2.id, segment3.id], 
    //@ts-ignore
    feature1Strategies[0].id);
    await addSegmentsToStrategy([segment2.id, segment3.id], 
    //@ts-ignore
    ...
    expect(collectIds(segmentStrategies3.strategies)).toEqual(collectIds([
        ...feature1Strategies,
        ...feature2Strategies,
        ...feature3Strategies,
    ]));
...
test('should reject duplicate segment names on update', async () => {
    ...
    await updateSegment(segmentB.id, {
        name: 'a',
        constraints: [],
    }, 409);
    await updateSegment(segmentA.id, {
...
describe('detect strategy usage in change requests', () => {
    ...
    const enterpriseFetchSegments = () => enterpriseApp.request
        .get(SEGMENTS_BASE_PATH)
        .expect(200)
        .then((res) => res.body.segments);
    const enterpriseFetchSegmentStrategies = (segmentId) => enterpriseApp.request
        .get(`${SEGMENTS_BASE_PATH}/${segmentId}/strategies`)
        .expect(200)
        .then((res) => res.body);
    beforeAll(async () => {
        enterpriseApp = await setupAppWithCustomConfig(db.stores, {
    ...
    test('Should show segment usage in updateStrategy events', async () => {
        ...
        await createFeatureFlag(enterpriseApp, flag);
        const [segment] = await enterpriseFetchSegments();
        await addStrategyToFeatureEnv(enterpriseApp, { ...flag.strategies[0] }, DEFAULT_ENV, flag.name);
        const [feature] = await fetchFeatures();
        const [strategy] = await fetchFeatureStrategies(feature.name);
...
//# sourceMappingURL=admin-segment.e2e.test.js.map