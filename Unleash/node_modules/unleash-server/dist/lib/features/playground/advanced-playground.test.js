...
const enableToggle = (featureName) => app.request
    .post(`/api/admin/projects/default/features/${featureName}/environments/${DEFAULT_ENV}/on`)
    .send({})
    .expect(200);
const disableToggle = (featureName) => app.request
    .post(`/api/admin/projects/default/features/${featureName}/environments/${DEFAULT_ENV}/off`)
    .send({})
    .expect(200);
afterAll(async () => {
    await app.destroy();
...
test('show matching variant from variants selection only for enabled toggles', async () => {
    ...
    const enabledFeatures = typedResult.features[0].environments[DEFAULT_ENV].filter((item) => item.isEnabled);
    const disabledFeatures = typedResult.features[0].environments[DEFAULT_ENV].filter((item) => !item.isEnabled);
...
test('should return disabled strategies with unevaluated result', async () => {
    await createFeatureToggleWithStrategy('test-playground-feature-with-disabled-strategy', {
        name: 'flexibleRollout',
        constraints: [],
        disabled: true,
        parameters: {
    ...
    const { body: result } = await app.request
        ...
        .expect(200);
    const typedResult = result;
    const feature = typedResult.features.find((feature) => feature.name === 'test-playground-feature-with-disabled-strategy');
    expect(feature?.environments[DEFAULT_ENV][0].strategies.data[0].result
        .evaluationStatus).toBe('unevaluated');
});
