...
import { CLIENT_METRICS, CLIENT_METRICS_ADDED, CLIENT_REGISTER, } from '../../../events/index.js';
...
import { generateDayBuckets, generateHourBuckets, } from '../../../util/time-utils.js';
import { nameSchema } from '../../../schema/feature-schema.js';
import memoizee from 'memoizee';
import { MetricsTranslator, } from '../impact/metrics-translator.js';
import { impactRegister } from '../impact/impact-register.js';
export default class ClientMetricsServiceV2 {
    constructor({ clientMetricsStoreV2 }, config, lastSeenService, unknownFlagsService) {
        this.unsavedMetrics = [];
        ...
        this.cachedFeatureNames = memoizee(async () => this.clientMetricsStoreV2.getFeatureFlagNames(), {
            promise: true,
    ...
    async filterExistingToggleNames(toggleNames) {
        ...
        const existingNames = toggleNames.filter((name) => existingFlags.includes(name));
        let unknownToggleNames = [];
        if (this.flagResolver.isEnabled('reportUnknownFlags')) {
            try {
                unknownToggleNames = toggleNames.filter((name) => !existingFlags.includes(name));
            }
            catch (e) {
                this.logger.error(e);
            }
        }
        const validatedToggleNames = await this.filterValidToggleNames(existingNames);
        return { validatedToggleNames, unknownToggleNames };
    }
    async filterValidToggleNames(toggleNames) {
        const nameValidations = toggleNames.map((toggleName) => nameSchema.validateAsync({ name: toggleName }));
    ...
    async siftMetrics(metrics) {
        if (!metrics.length)
            return [];
        const metricsByToggle = new Map();
        ...
        if (metricsByToggle.size === 0)
            return [];
        const toggleNames = Array.from(metricsByToggle.keys());
        const { validatedToggleNames, unknownToggleNames } = await this.filterExistingToggleNames(toggleNames);
        ...
        this.logger.debug(`Got ${toggleNames.length} metrics (${invalidCount > 0 ? `${invalidCount} invalid` : 'all valid'}).`);
        const unknownFlags = [];
        ...
        if (unknownFlags.length) {
            const sample = unknownFlags
                ...
                .map((f) => `"${f.name}"`)
                .join(', ');
            this.logger.debug(`Registering ${unknownFlags.length} unknown flags; sample: ${sample}`);
    ...
    async registerImpactMetrics(impactMetrics) {
        try {
    ...
    async registerClientMetrics(data, clientIp) {
        ...
        const clientMetrics = Object.keys(value.bucket.toggles).map((name) => ({
            featureName: name,
        ...
        const result = environments.flatMap((environment) => {
            ...
            return applications.flatMap((appName) => {
                ...
                return hours.flatMap((hourBucket) => {
                    const metric = applicationMetrics.find((item) => compareAsc(hourBucket.timestamp, item.timestamp) ===
                        0);
                    return (metric || {
