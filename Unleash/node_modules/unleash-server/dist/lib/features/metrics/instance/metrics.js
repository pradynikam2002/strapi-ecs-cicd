...
import { clientMetricsEnvBulkSchema, customMetricsSchema, } from '../shared/schema.js';
export default class ClientMetricsController extends Controller {
    constructor({ clientInstanceService, clientMetricsServiceV2, openApiService, customMetricsService, }, config) {
        super(config);
    ...
    async registerMetrics(req, res) {
        if (this.config.flagResolver.isEnabled('disableMetrics')) {
            ...
            try {
                ...
                await this.clientInstanceService.registerInstance(metricsData, clientIp);
                await this.metricsV2.registerClientMetrics(metricsData, clientIp);
        ...
        if (this.config.flagResolver.isEnabled('disableMetrics')) {
            ...
            try {
                ...
                if (body.metrics && Array.isArray(body.metrics)) {
                    const validMetrics = body.metrics.filter((metric) => typeof metric.name === 'string' &&
                        typeof metric.value === 'number');
                    if (validMetrics.length < body.metrics.length) {
                        this.logger.warn('Some invalid metric types found, skipping');
                    }
                    this.customMetricsService.addMetrics(validMetrics);
                }
                res.status(202).end();
            }
            catch (e) {
            ...
            try {
                ...
                for (const app of applications) {
                    if (app.sdkType === 'frontend' &&
                        ...
                        promises.push(this.clientInstanceService.registerBackendClient(app, clientIp));
                    }
                }
                if (metrics && metrics.length > 0) {
                    const data = await clientMetricsEnvBulkSchema.validateAsync(metrics);
                    const { user } = req;
                ...
                if (this.flagResolver.isEnabled('impactMetrics') &&
                    ...
                    promises.push(this.metricsV2.registerImpactMetrics(impactMetrics));
                }
