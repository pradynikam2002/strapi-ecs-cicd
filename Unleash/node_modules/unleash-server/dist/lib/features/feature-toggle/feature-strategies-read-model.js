export class FeatureStrategiesReadModel {
    constructor(db) {
        this.db = db;
    }
    activeStrategies() {
    ...
    async getMaxFeatureEnvironmentStrategies() {
    ...
    async getMaxConstraintValues() {
        const rows = await this.activeStrategies()
            .select('feature_name', 'environment', this.db.raw("MAX(coalesce(jsonb_array_length(constraint_value->'values'), 0)) as max_values_count"))
    ...
    async getMaxConstraintsPerStrategy() {
        const rows = await this.activeStrategies()
...
//# sourceMappingURL=feature-strategies-read-model.js.map