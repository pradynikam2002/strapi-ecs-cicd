...
import { FEATURE_ENVIRONMENT_DISABLED, FEATURE_ENVIRONMENT_ENABLED, FEATURE_METADATA_UPDATED, FEATURE_STALE_OFF, FEATURE_STALE_ON, FEATURE_STRATEGY_REMOVE, } from '../../../events/index.js';
...
const createStrategy = async (featureName, payload, expectedCode = 200) => {
    return app.request
        .post(`/api/admin/projects/default/features/${featureName}/environments/${DEFAULT_ENV}/strategies`)
        .send(payload)
        .expect(expectedCode);
};
const updateStrategy = async (featureName, strategyId, payload, expectedCode = 200) => {
    const { body } = await app.request
        .put(`/api/admin/projects/default/features/${featureName}/environments/${DEFAULT_ENV}/strategies/${strategyId}`)
        .send(payload)
        .expect(expectedCode);
    return body;
};
...
test('Should archive feature flag', async () => {
    ...
    const { body } = await app.request
        .get(`/api/admin/search/features?project=IS%3A${projectId}&archived=IS%3Atrue`)
        .expect(200);
...
test('Deleting last strategy for feature environment should disable that environment', async () => {
    ...
    await app.request.delete(`/api/admin/projects/default/features/${featureName}/environments/${envName}/strategies/${strategyId}`);
    await app.request
        .get(`/api/admin/projects/default/features/${featureName}/environments/${envName}`)
        .expect(200)
        .expect((res) => {
...
test('Should not allow changing project to target project without the same enabled environments', async () => {
    ...
    await db.stores.projectStore.addEnvironmentToProject(targetProject, DEFAULT_ENV);
    await app.createFeature(featureName, project);
    await app.request
        .post(`/api/admin/projects/${project}/features/${featureName}/environments/${DEFAULT_ENV}/strategies`)
        .send({
        name: 'flexibleRollout',
        parameters: {
            groupId: featureName,
    ...
    await app.request
        ...
        .expect(200);
    await app.request
        .post(`/api/admin/projects/${project}/features/${featureName}/environments/${DEFAULT_ENV}/on`)
        .send({})
        .expect(200);
    await app.request
    ...
    await expect(async () => app.services.projectService.changeProject(targetProject, featureName, 
    //@ts-ignore
    user, 'default', TEST_AUDIT_USER)).rejects.errorWithMessage(new IncompatibleProjectError(targetProject));
});
test('Should allow changing project to target project with the same enabled environments', async () => {
    ...
    await expect(app.services.projectService.changeProject(targetProject, featureName, 
    //@ts-ignore
    user, 'default', TEST_AUDIT_USER)).resolves;
});
...
test('should add default constraint values for single-valued constraints', async () => {
    ...
    const mockStrategy = (constraint) => ({
        name: 'flexibleRollout',
        constraints: [constraint],
    });
    const expectValues = (res, values) => {
...
test('should allow long parameter values', async () => {
    ...
    await app.request
        .post(`/api/admin/projects/${project.id}/features/${flag.name}/environments/${DEFAULT_ENV}/strategies`)
        .send(strategy)
        .expect(200);
});
test('should change strategy sort order when payload is valid', async () => {
    ...
    const { body: strategyOne } = await app.request
        .post(`/api/admin/projects/default/features/${flag.name}/environments/${DEFAULT_ENV}/strategies`)
        .send({
        name: 'default',
        parameters: {
            userId: 'string',
    ...
    const { body: strategyTwo } = await app.request
        ...
        .expect(200);
    const { body: strategies } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/${DEFAULT_ENV}/strategies`);
    expect(strategies[0].sortOrder).toBe(0);
    ...
    await app.request
        ...
        .expect(200);
    const { body: strategiesOrdered } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/${DEFAULT_ENV}/strategies`);
...
test('should return strategies in correct order when new strategies are added', async () => {
    ...
    await app.request
        ...
        .expect(200);
    const { body: strategiesReOrdered } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/${DEFAULT_ENV}/strategies`);
...
test('should create a strategy with segments', async () => {
    ...
    await app.request
        ...
        .expect((res) => {
        const defaultEnv = res.body.environments.find((env) => env.name === DEFAULT_ENV);
...
test('should disable strategies in place', async () => {
    ...
    const { body: updatedStrategyOne } = await app.request
        .put(`/api/admin/projects/default/features/${flag.name}/environments/${DEFAULT_ENV}/strategies/${strategyOne.id}`)
        .send({
        name: 'flexibleRollout',
        constraints: [],
        disabled: true,
        ...
        .expect(200);
    const { body: updatedStrategies } = await app.request.get(`/api/admin/projects/default/features/${flag.name}/environments/${DEFAULT_ENV}/strategies`);
...
//# sourceMappingURL=feature-toggles.e2e.test.js.map